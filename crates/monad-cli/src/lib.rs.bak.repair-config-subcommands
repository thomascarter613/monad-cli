...
fn list_cmd() -> Command {
    Command::new("list")
        ...
        .arg(opt(
            "scope",
            "Optional scope filter reserved for future workspace units",
        ))
...
fn inspect_cmd() -> Command {
    Command::new("inspect")
        .about(
            "Inspect workspace structure, manifests, crates, command catalog, and managed surfaces",
        )
...
fn build_config_output() -> ConfigCommandOutput {
    ...
    let entries = vec![
        ...
        ConfigEntry {
            ...
            source: "approved-default",
            description: "Default package manager for generated JavaScript/TypeScript surfaces."
                .to_string(),
        },
        ConfigEntry {
...
fn discover_monad_workspace_root() -> std::path::PathBuf {
...
fn list_manifest_items(root: &std::path::Path) -> Vec<ListItem> {
    [
        ...
        (
            ".monorepo.json",
            "monorepo compatibility/introspection metadata",
        ),
    ]
    .iter()
...
fn list_crate_items(root: &std::path::Path) -> Vec<ListItem> {
    ...
    for entry in entries.flatten() {
        ...
        let cargo_contents =
            std::fs::read_to_string(crate_dir.join("Cargo.toml")).unwrap_or_default();
...
fn list_surface_items(root: &std::path::Path) -> Vec<ListItem> {
    [
        (
            "governance",
            "governance model, decision records, and operating rules",
        ),
        ("domains", "domain ownership and bounded-context surfaces"),
        (
            "ops",
            "operations, runbooks, and production readiness surfaces",
        ),
        (
            "observability",
            "metrics, logs, traces, and telemetry surfaces",
        ),
        ("compliance", "compliance evidence and controls"),
        ("security", "security policy, threat model, and controls"),
        (
            "environments",
            "environment definitions and deployment topology",
        ),
